...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  const newArr = arr.filter(data => {
    return data.match(/[a,e,i,o,u]/gm) ? true : false;
  });
  return newArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const newArr = arr.filter((value) => !forbiddenValues.includes(value));
  return newArr;
};
