...
const containsWorld = (input) => {
  // Solution code here...
  const answer = /world/gm.test(input);
...
const isCapitalized = (str) => {
  // Solution code here...
  const newStringArr = str.match(/[A-Z]\w+/gm);
  if (newStringArr){
    return newStringArr;
  }else{
    return [];
...
const citiesAtoJ = (arr) => {
  ...
  const newCityArray = [];
  arr.forEach(value => {
    const newString = value.match(/^[A-J]\w+/gm);
    if (newString){
      newCityArray.push(newString[0]);
    }
  });
  return newCityArray;
};
...
const matchMonth = (input) => {
  ...
  const answer1 = /\bOct\b/gm.test(input);
  const answer2 = /\boct\b/gm.test(input);
  const answer3 = /\bOctober\b/gm.test(input);
  const answer4 = /\boctober\b/gm.test(input);
  if (answer1) {
    return true;
  }else if (answer2) {
    return true;
...
const noPunctuation = str => {
  // Solution code here...
  const stringAnswer = str.match(/\w+\s/gm);
  return stringAnswer;
};
...
let hangman = (str) => {
  // Solution code here...
  let re = /[aeiouAEIOU]/gm;
  let newString = str.replace(re, '_');
  return newString;
};
...
const findShells = (str) => {
  ...
  const stringAnswer = str.match(/\w+ells/gm);
  return stringAnswer;
};
