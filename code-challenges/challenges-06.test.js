...
const getCourseKeys = (obj) => {
  // Solution code here...
  const keysFromObject = Object.keys(obj);
...
const updateNumbers = (obj) => {
  ...
  // console.log(Object.entries(objectOfNumbers));
  var newArr = (Object.entries(obj));
  var results = [];
  for (let i in newArr) {
    results.push(newArr[i][0] + ': ' + newArr[i][1])
  }
  return results;
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here..
  for (let i in arr) {
    var newArr = Object.values(arr[i].children);
    console.log(newArr);
  }
  for (let j in newArr) {
    if (Object.values(newArr[j].indexOf(character) > -1)) {
      return true;
      //   console.log(arr[i]);
    } else {
      return false;
