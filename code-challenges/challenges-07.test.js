...
const replaceZeros = (string) => {
  // Solution code here...
  const str = string.replace(/0/gm, 'zero');
  return str;
};
...
const addOne = (arr) => {
  ...
  const answer = arr.map(addOneToIt);
  function addOneToIt(data) {
    return data + 1;
  }
  return answer;
};
...
const addQuestion = (arr) => {
  // Solution code here...
  const answer = arr.map(value => value + "?")
  return answer;
};
...
const mapTwoToThe = (arr) => {
  // Solution code here...
  const answer = arr.map(value => Math.pow(2, value));
  return answer;
};
