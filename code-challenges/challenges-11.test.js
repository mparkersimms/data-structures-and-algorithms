...
const toLastNames = people => {
  // Solution code here...
  const peepArr = people.map(value => {
...
const validateWord = (word) => {
  // Solution code here...
  return (/^[a-zA-Z]{5,10}$/gm).test(word);
};
...
const hasNumber = (string) => {
  // Solution code here...
  return (/^[a-zA-Z]+[0-9]+/gm).test(string);
};
...
const validateEmail = (email) => {
  // Solution code here...
  return (/^(\w+[.])?\w+@\w+\.(net|com|org)$/gm).test(email);
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  return (/\(?\d{3}\)?[\s\-]?\d{3}[\s\-]?\d{4}$/gm).test(phoneNumber);
